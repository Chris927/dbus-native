...
function write(ps, ele, data) {
  // console.log(`write, ele=${JSON.stringify(ele)}, data=${data}`);
  switch (ele.type) {
    case '(':
    case '{':
      align(ps, 8);
    ...
    case 'v':
      ...
      if (data[1] === null) {
        // write a null value as empty int array
        write(ps, signatureEle, 'ai');
        write(ps, { type: 'a', child: [{ type: 'i' }] }, []); // write an empty int array, to represent null
      } else {
...
function writeSimple(ps, type, data) {
  // console.log(`writeSimple: type=${JSON.stringify(type)}, data=${data}`)
  if (typeof data === 'undefined')
    throw new Error(
      "Serialisation of JS 'undefined' type is not supported by d-bus"
    );
  // console.log(`writeSimple, ps=${ps}, type=${type}, data=${data}`)
  if (data === null) {
    // TODO: this might be too late, as the type information (about the value being a double) might be written already.
    // write(ps, { type: 'a', child: [{ type: 'i' }] }, []); // write an empty int array, to represent null
    // return ps;
  ...
  // console.log(`writeSimple, about to marshall, type=${type}, data=${data}`);
