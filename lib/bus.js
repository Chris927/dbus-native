...
module.exports = function bus(conn, opts, cb) {
  if (!(this instanceof bus)) {
    return new bus(conn, opts, cb);
  }
  if (!opts) opts = {};
  ...
  // register name
  // TODO: note how we use 'cb' as callback: We ensure that, despite the value of opts.direct, the
  // callback is always called with (err, result), where err is null if there is no error.
  // This ensures the caller can rely on the callback being called.
  if (opts.direct !== true) {
    this.invokeDbus({ member: 'Hello' }, function(err, name) {
      // TODO: if err is truthy, then this crashes node-red.
      // Long-term, we should make exportInterface async.
      if (err) {
        if (cb) {
          cb(err);
          return;
