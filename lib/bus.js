  ...
  this.connection.on('message', function(msg) {
    ...
    if (
      ...
      if ((obj = self.exportedObjects[msg.path])) {
        console.log(
          `CHRIS-DEBUG msg['interface'], msg['body']:`,
          msg['interface'],
          msg['body']
        );
        if ((iface = obj[msg['interface']])) {
          ...
          // TODO: NO, try and find a matching method
          for (const ifaceName of Object.keys(obj)) {
            iface = obj[ifaceName];
            console.log(
              `[CHRIS-DEBUG] Checking interface ${ifaceName}: iface=${JSON.stringify(
                iface
              )}`
            );
            for (const methodName of Object.keys(iface[0].methods)) {
              if (methodName === msg.member) {
                console.log('[CHRIS-DEBUG] FOUND!');
                const resultSignature = iface[0].methods[msg.member][1];
                impl = iface[1];
                const func = impl[msg.member];
                console.log(`[CHRIS-DEBUG] impl=${impl}, func=${func}`);
                invoke(impl, func, resultSignature);
              }
            }
          }
