...
module.exports = function bus(conn, opts, constructorCallback) {
  ...
  this.invoke = function(msg, callback) {
    ...
    self.connection.message(msg, function(err) {
      if (err) {
        // debug(
        //   `Error sending message: ${JSON.stringify(msg)}`, err
        // );
        delete self.cookies[msg.serial];
        // TODO: we *only* call the callback if there is an error.
        // This is probably not the intention. When I tried to call
        // the callback in the non-error case,
        // node-red-contrib-victron crashes.
  ...
  this.connection.on('message', function(msg) {
    debugLogAllMessages('DBUS MESSAGE', msg);
    if (msg && msg.errorName) {
      console.error('DBUS ERROR MESSAGE', msg.errorName);
    }
    ...
    if (
      ...
      if ((obj = self.exportedObjects[msg.path])) {
        if ((iface = obj[msg['interface']])) {
          ...
          // debug(
          //   `[CHRIS-DEBUG] Interface ${
          //     msg['interface']
          //   } is not supported, msg=${JSON.stringify(
          //     msg
    ...
    if (typeof obj.emit === 'function') {
      ...
      obj.emit = function() {
        ...
        if (iface.signals && iface.signals[signalName]) {
          ...
          console.error(
            `Trying to emit unknown signal ${signalName} on interface ${
              iface.name
            }`
          );
        }
  ...
  function doCallback(err, result) {
    if (typeof constructorCallback === 'function') {
      ...
      // we throw, possibly from a callback. This may crash the process, as our caller may not
      // be able to catch this error. This is the old behavior, which we preserve for backwards
      // compatibility.
      throw new Error(err);
    }
  }
  ...
  // register name
  // TODO: note how we use 'constructorCallback': We ensure that, despite the value of opts.direct, the
  // callback is always called with (err, result), where err is null if there is no error. This ensures
  // the caller can rely on the callback being called.
  if (opts.direct !== true) {
    this.invokeDbus({ member: 'Hello' }, function(err, name) {
      // TODO: if err is truthy, then this crashes node-red, unless there is a callback.
      if (err) {
