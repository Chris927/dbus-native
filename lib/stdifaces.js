const debug = require('debug')('dbus-native-victron');
...
function respondToInsprospect(msg, bus) {
  const resultXml = [xmlHeader];

  resultXml.push(`<node name="${msg.path}">`);
  resultXml.push(stdIfaces);
  const objectAtPath = bus.exportedObjects[msg.path];
  if (objectAtPath) {
    debug(
      'need to add all interfaces, path, exportedObject',
      msg.path,
      objectAtPath
    );
    for (const iface in objectAtPath) {
      debug('iface, objectAtPath[iface]', iface, objectAtPath[iface]);
      resultXml.push(interfaceToXML(objectAtPath[iface][0]));
    }
  }
  resultXml.push('</node>');
  ...
  debug(`stdifaces, introspectableReply:`, introspectableReply);
...
module.exports = function(msg, bus) {
  if (
    ...
    return respondToInsprospect(msg, bus);
